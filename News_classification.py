...
(train_data, train_labels), (test_data, test_labels) =reuters.load_data(path=paths, num_words=10000)
...
print('results: loss: ', results[0],'acc:',results[2])

test_labels_copy = copy.copy(test_labels)
np.random.shuffle(test_labels_copy)
hits_array = np.array(test_labels) == np.array(test_labels_copy)
print('random acc:', float(np.sum(hits_array)) / len(test_labels))
